...
extern "C"
void multi_gpus(std::string cur_dir);
...
void gray_processing_multi_gpus(unsigned char* color_pixels, 
	...
	//for( int i=0; i<device_cnt; i++ ){
	//	std::cout<<"Pixel size for device "<<i<<" is: "<<device_pixel_cnt[i]<<std::endl;
	//}
...
void multi_gpus(std::string cur_dir){
	...
	unsigned long int max_pixels_cnt = max_mem/8;
	std::cout<<"Maximul pixels number: "<< max_pixels_cnt<<std::endl;
	std::cout<<"Maximul memory size: "<< (double)(max_pixels_cnt*(BYTES_PER_PIXEL+1))/(double)(1024*1024*1024)<<std::endl<<std::endl;
	...
	while((img_ent = readdir(img_src))){
		...
		if(strcmp(img_ent->d_name, ".") == 0 || strcmp(img_ent->d_name, "..")==0)
			...
			if( process_pixels_cnt + img.total() <= max_pixels_cnt ){
				...
				// GPU processing
				gray_processing_multi_gpus(h_color_img_pixels, h_gray_img_pixels, process_pixels_cnt, device_cnt);
				...
				process_pixels_cnt = img.total();
				std::cout<<"Collecting images pixels......"<<std::endl<<std::endl;
			}
		}
	}
